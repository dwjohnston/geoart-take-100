...
type UserPreferencesContextType = {
    getPreference: <T extends keyof UserPreferences>(key: T) => UserPreferences[T] | null; 
    setPreference: <T extends keyof UserPreferences>(key: T, value: UserPreferences[T]) => UserPreferences[T]; 
...
export const UserPreferencesContextProvider = (
  ...
  useEffect(() => {
    ...
    Object.entries(initialPreferences).forEach(([_key,value]) => {
        ...
        const valueFromLocalStorage = window.localStorage.getItem(prefix+ "__" + key); 
  ...
  const getUserPreference = <T extends keyof UserPreferences>(key:T ) : UserPreferences[T] | null => {
    return inMemoryState[key]; 
  }

  const setUserPreference = <T extends keyof UserPreferences>(key:T, value: UserPreferences[T] ) : UserPreferences[T] => {
    setInMemoryState((state) => {
